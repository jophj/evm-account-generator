...
fn main() {
    ...
    let private_key = EvmKey::new(&[0x12u8; 32]).expect("Valid key");
    ...
    let private_key: EvmKey = thread_rng_generator.generate();
    println!("   Private Key: {}", private_key.to_string());
    println!("   Address:     {}", private_key.derive_address());
    println!();
    ...
    let private_key_dev: EvmKey = dev_random_generator.generate();
    ...
    match EvmKey::from_string(hex_str) {
        Some(key) => {
    ...
    match EvmKey::from_string("0x123") {
    ...
    match EvmKey::from_string("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdefg") {
        Some(key) => println!("✗ Unexpected success: {}", key.to_string()),
